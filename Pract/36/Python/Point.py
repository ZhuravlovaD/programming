...
class Coordinata(Enum):
    Cartesian = auto()
    Polar = auto()


class Point:
    def __init__(self, a1=0, a2=0, coord_system=Coordinata.Cartesian):
        if type(a1) != str:
            if Coordinata.Cartesian:
                self.x = a1
                self.y = a2
            else:
            ...
            comma = a1.find(',')
            self.x = float(a1[1:comma].strip())
            self.y = float(a1[comma + 1:-2].strip())
